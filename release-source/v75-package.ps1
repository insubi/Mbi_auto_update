...
# Reuse the already-proven V0.1.1/v74 packaging path, changing only version/audit identifiers.
...
$text = $text.Replace('Abyss false-clear guard requires clear-title + touch-prompt together for 3 consecutive frames',
    'Abyss stability: BOTH images in one frame then immediate touch, state-aware recovery, verified exit, repaired monitors')
...
# Verify the repaired runtime images in both executable and mirror paths.
...
foreach ($rel in $requiredImages) {
    ...
    if (-not (Test-Path -LiteralPath $path)) { throw "V0.1.2 repaired runtime image missing: $rel" }
    $img = $null
    try {
...
# Re-create package ZIP/inventory after the final integrity checks so published hash covers the exact verified tree.
