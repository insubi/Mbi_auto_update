...
& (Join-Path $PSScriptRoot 'v74-verify.ps1') -SourceRoot $SourceRoot
...
$text = $text.Replace('v66','v74')
$text = $text.Replace('V66_ABYSS_CLEAR_AUTOSTOP_AUDIT.json','V0_1_1_ABYSS_CLEAR_GUARD_AUDIT.json')
$text = $text.Replace("if (`$v74Audit.version -ne 'v74')", "if (`$v74Audit.version -ne 'V0.1.1')")
$text = $text.Replace('66.0.0.0','0.1.1.0')
$text = $text.Replace('v74 updates:', 'V0.1.1 updates: Abyss false-clear guard requires clear-title + touch-prompt together for 3 consecutive frames')
...
$tmp = Join-Path $env:TEMP ('mabiauto-v74-package-' + [Guid]::NewGuid().ToString('N') + '.ps1')
try {
    Set-Content -LiteralPath $tmp -Value $text -Encoding UTF8
    & $tmp -SourceRoot $SourceRoot -OutputRoot $OutputRoot
    if ($LASTEXITCODE -ne 0) { throw "V0.1.1 package delegate failed: $LASTEXITCODE" }
} finally {
    Remove-Item -LiteralPath $tmp -Force -ErrorAction SilentlyContinue
}

$liteRoot = Join-Path $OutputRoot 'MabiAuto_v74'
...
foreach ($destBase in @(
    ...
    foreach ($entry in $expected.GetEnumerator()) {
        ...
        if (-not (Test-Path -LiteralPath $path)) { throw "V0.1.1 preview package path missing: $path" }
        if ((Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash.ToLowerInvariant() -ne $entry.Value) {
            throw "V0.1.1 preview package hash mismatch: $path"
        }
    }
}

if (Test-Path -LiteralPath (Join-Path $SourceRoot 'VERSIONING_V0.1.txt')) {
    Copy-Item -LiteralPath (Join-Path $SourceRoot 'VERSIONING_V0.1.txt') -Destination (Join-Path $liteRoot 'VERSIONING.txt') -Force
}

$zip = Join-Path $OutputRoot 'MabiAuto_v74_Windows_Lite.zip'
...
"$hash  MabiAuto_v74_Windows_Lite.zip" | Set-Content -LiteralPath "$zip.sha256" -Encoding ASCII
...
$inventory | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath (Join-Path $OutputRoot 'v74-package-inventory.json') -Encoding UTF8
Write-Host "V0.1.1 PACKAGE VERIFIED (legacy bridge v74): $($inventory.Count) files; SHA256 $hash"
