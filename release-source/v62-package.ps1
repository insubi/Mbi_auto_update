...
$v62AuditPath = Join-Path $SourceRoot 'V62_WINDOW_FORCE_AUDIT.json'
if (-not (Test-Path -LiteralPath $v62AuditPath)) { throw 'V62 window-force audit missing' }
$v62Audit = Get-Content -LiteralPath $v62AuditPath -Raw | ConvertFrom-Json
if ($v62Audit.version -ne 'v62') { throw 'Expected audited v62 source' }
foreach ($entry in $v62Audit.files.PSObject.Properties) {
    $path = Join-Path $SourceRoot $entry.Name
    if (-not (Test-Path -LiteralPath $path)) { throw "Audited v62 file missing: $($entry.Name)" }
    if ((Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash.ToLowerInvariant() -ne $entry.Value.ToLowerInvariant()) {
        throw "v62 source integrity mismatch: $($entry.Name)"
    }
}

$v58AuditPath = Join-Path $SourceRoot 'V58_PACKAGING_AUDIT.json'
if (-not (Test-Path -LiteralPath $v58AuditPath)) { throw 'V58 packaging audit missing from v62 source' }
...
$liteRoot = Join-Path $OutputRoot 'MabiAuto_v62'
...
@'
MABI AUTO v62 - Windows Lite

v62 strengthens game-window selection and resizing:
- prefer the visible '마비노기 모바일' window
- force normal restore bounds before resizing
- SetWindowPos + MoveWindow fallback
- actual client area target 800x1000 on the primary monitor top-right
- extra diagnostics if Windows/game still refuses the resize
Fishing, Dungeon and Abyss keep the same required geometry.
Updater/watchdog, OCR/retry logic and v58 safe packaging remain intact.
...
if ($version -ne '62.0.0.0') { throw "Unexpected executable version: $version" }
...
try {
    ...
    Write-Host 'STARTUP HEALTH OK: v62 WinForms app and Watchdog started.'
} finally {
...
$zip = Join-Path $OutputRoot 'MabiAuto_v62_Windows_Lite.zip'
Compress-Archive -LiteralPath $liteRoot -DestinationPath $zip -CompressionLevel Optimal
$hash = (Get-FileHash -LiteralPath $zip -Algorithm SHA256).Hash.ToLowerInvariant()
"$hash  MabiAuto_v62_Windows_Lite.zip" | Set-Content -LiteralPath "$zip.sha256" -Encoding ASCII
...
$inventory | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath (Join-Path $OutputRoot 'v62-package-inventory.json') -Encoding UTF8
