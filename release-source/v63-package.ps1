...
$v63AuditPath = Join-Path $SourceRoot 'V63_ADMIN_WINDOW_CLEANUP_AUDIT.json'
if (-not (Test-Path -LiteralPath $v63AuditPath)) { throw 'V63 admin/window cleanup audit missing' }
$v63Audit = Get-Content -LiteralPath $v63AuditPath -Raw | ConvertFrom-Json
if ($v63Audit.version -ne 'v63') { throw 'Expected audited v63 source' }
foreach ($entry in $v63Audit.files.PSObject.Properties) {
    $path = Join-Path $SourceRoot $entry.Name
    if (-not (Test-Path -LiteralPath $path)) { throw "Audited v63 file missing: $($entry.Name)" }
    if ((Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash.ToLowerInvariant() -ne $entry.Value.ToLowerInvariant()) {
        throw "v63 source integrity mismatch: $($entry.Name)"
    }
}

$v58AuditPath = Join-Path $SourceRoot 'V58_PACKAGING_AUDIT.json'
if (-not (Test-Path -LiteralPath $v58AuditPath)) { throw 'V58 packaging audit missing from v63 source' }
...
$liteRoot = Join-Path $OutputRoot 'MabiAuto_v63'
...
@'
MABI AUTO v63 - Windows Lite

v63 cleanup/stability release:
- MABI AUTO now requests administrator privileges automatically at launch
- keeps actual game client target at 800x1000
- keeps primary-monitor top-right placement and dual-monitor handling
- keeps visible game-window selection from v62
- removes repeated MoveWindow/window-state fallback behavior and noisy resize retry logs
- keeps updater/watchdog, OCR/retry logic and v58 safe packaging unchanged

Windows will show a UAC confirmation when MABI AUTO starts. This is required because the game itself can run elevated and Windows otherwise blocks window resizing with Win32 error 5 (Access Denied).
...
if ($version -ne '63.0.0.0') { throw "Unexpected executable version: $version" }
...
try {
    ...
    Write-Host 'STARTUP HEALTH OK: v63 WinForms app and Watchdog started.'
} finally {
...
$zip = Join-Path $OutputRoot 'MabiAuto_v63_Windows_Lite.zip'
Compress-Archive -LiteralPath $liteRoot -DestinationPath $zip -CompressionLevel Optimal
$hash = (Get-FileHash -LiteralPath $zip -Algorithm SHA256).Hash.ToLowerInvariant()
"$hash  MabiAuto_v63_Windows_Lite.zip" | Set-Content -LiteralPath "$zip.sha256" -Encoding ASCII
...
$inventory | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath (Join-Path $OutputRoot 'v63-package-inventory.json') -Encoding UTF8
