...
& (Join-Path $PSScriptRoot 'v76-verify.ps1') -SourceRoot $SourceRoot
...
$text = $text.Replace("V0.1.2","V0.1.3")
$text = $text.Replace("V0_1_2_STABILITY_AUDIT.json","V0_1_3_FISHING_AUDIT.json")
$text = $text.Replace("0.1.2.0","0.1.3.0")
$tmp = Join-Path $PSScriptRoot ('mabiauto-v76-package-' + [Guid]::NewGuid().ToString('N') + '.ps1')
try {
    Set-Content -LiteralPath $tmp -Value $text -Encoding UTF8
    & $tmp -SourceRoot $SourceRoot -OutputRoot $OutputRoot
    if ($LASTEXITCODE -ne 0) { throw "V0.1.3 package delegate failed: $LASTEXITCODE" }
} finally { Remove-Item -LiteralPath $tmp -Force -ErrorAction SilentlyContinue }
$liteRoot = Join-Path $OutputRoot 'MabiAuto_v76'
...
foreach ($path in @($sourceHook,$runtimeHook)) {
    if (-not (Test-Path -LiteralPath $path)) { throw "V0.1.3 packaged hook missing: $path" }
    $hash = (Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash.ToLowerInvariant()
    if ($hash -ne 'b85d3443d156ec31957a7b24ddeeb507a1c5bbbcbf2d3dad4be8213114b4ce8d') { throw "V0.1.3 packaged hook mismatch: $path" }
}
Write-Host 'V0.1.3 PACKAGE VERIFIED: current hook exists identically in source/runtime and START preserves runtime templates.'
