...
& (Join-Path $PSScriptRoot 'v70-verify.ps1') -SourceRoot $SourceRoot
...
$text = $text.Replace('v66','v70')
$text = $text.Replace('V66_ABYSS_CLEAR_AUTOSTOP_AUDIT.json','V70_AUTOSTOP_UI_AUDIT.json')
$text = $text.Replace('66.0.0.0','70.0.0.0')
$text = $text.Replace('v70 updates:', 'v70 updates: polished HOME scheduled-stop control; v69 Telegram/Abyss UI and recovery preserved')

# Keep the v69 parent-PID Watchdog verification because unrelated machine-wide
# MacroWatchdog processes may already exist on the build runner/user PC.
$newWatchdogCheck = @'
...
$tmp = Join-Path $env:TEMP ('mabiauto-v70-package-' + [Guid]::NewGuid().ToString('N') + '.ps1')
try {
    Set-Content -LiteralPath $tmp -Value $text -Encoding UTF8
    & $tmp -SourceRoot $SourceRoot -OutputRoot $OutputRoot
    if ($LASTEXITCODE -ne 0) { throw "v70 package delegate failed: $LASTEXITCODE" }
} finally {
    Remove-Item -LiteralPath $tmp -Force -ErrorAction SilentlyContinue
}

$liteRoot = Join-Path $OutputRoot 'MabiAuto_v70'
foreach ($rel in @(
    ...
    if (-not (Test-Path -LiteralPath (Join-Path $liteRoot $rel))) { throw "v70 package path missing: $rel" }
}
Write-Host 'V70 PACKAGE VERIFIED: polished auto-stop UI plus all v69 Abyss previews/updater/Watchdog paths included.'
