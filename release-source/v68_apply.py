...
# Verify the exact image-only targets needed by the recovery state machine.
...
for target_id in ("abyss_touch_screen", "abyss_dungeon_clear_visual", "treasure_chest", "abyss_exit", "abyss_menu"):
...
changes_path.write_text(
    ...
    "- Recovery sequence: clear/touch screen -> treasure chest -> exit -> outside menu confirmation.\n"
    "- Clear-title recognition is never clicked; only the actual touch-screen prompt image is clicked.\n"
    "- If treasure was already handled, recovery may continue directly from the exit screen.\n"
    ...
    "- v67 clear transition verification, v65 popup-close guard, auto-stop UI, treasure/exit targets, other UI/layouts and window sizing are unchanged.\n",
    encoding="utf-8",
)
...
audit = {
    "base": "v67",
    "version": "v68",
    "purpose": "Abyss in-dungeon recovery through touch, treasure and exit",
    "abyss_recovery": {
        "esc_used": False,
        "sequence": ["clear/touch screen", "treasure_chest", "abyss_exit", "abyss_menu outside confirmation"],
...
print("v68 applied: Abyss internal recovery = touch -> treasure -> exit -> outside confirmation")
