...
def replace_once(path: Path, old: str, new: str) -> None:
    s = read_text(path)
    if old not in s:
        raise RuntimeError(f"required pattern missing in {path}: {old}")
    write_text(path, s.replace(old, new, 1))

# ---------------------------------------------------------------------------
# v48 is UI-only on top of v47/v46 gameplay. Verify the working runtime pieces
...
if "WatchdogClient" not in main_form or "_watchdog.Touch()" not in main_form:
    raise RuntimeError("watchdog integration missing from v47 base")
if "CheckForUpdatesAsync(false)" not in main_form:
    raise RuntimeError("automatic update check missing from v47 base")
...
# Version and window geometry only. No scenario/target/runtime logic is changed.
replace_once(app / "UpdateManager.cs", 'public const string CurrentVersion = "v47";', 'public const string CurrentVersion = "v48";')
replace_once(app / "MainForm.cs", "Width = 1100;", "Width = 1320;")
replace_once(app / "MainForm.cs", "Height = 760;", "Height = 900;")
replace_once(app / "MainForm.cs", "MinimumSize = new Size(1020, 720);", "MinimumSize = new Size(1180, 800);")
replace_once(app / "MainForm.cs", "Padding = new Padding(8);", "Padding = new Padding(0);")

# ---------------------------------------------------------------------------
# Replace dashboard presentation only. Existing StartSelected/StopSelected,
# status, logs, diagnostics, updater and scenario engine are reused unchanged.
# Potion popup close stays background-only and is intentionally not shown here.
# ---------------------------------------------------------------------------
dashboard = r'''using DungeonVisionBot;
...
public sealed partial class MainForm
{
    ...
    {
        var header = new Panel { Dock = DockStyle.Fill, BackColor = NavBg, Padding = new Padding(22, 10, 18, 10) };
        ...
        brand.Controls.Add(new Label { Text = "v48.0.0", Dock = DockStyle.Fill, ForeColor = Muted, Font = new Font("Segoe UI", 9f), TextAlign = ContentAlignment.MiddleLeft }, 1, 0);
    ...
    private Control StatusChip(Label label, string caption)
    {
        ...
        label.Font = new Font("맑은 고딕", 8.7f, FontStyle.Bold);
    ...
    private TableLayoutPanel BuildDashboardGrid()
    {
        ...
        grid.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 76));
    ...
    {
        var card = BorderedPanel(Color.FromArgb(9, 34, 62));
    ...
    private Control BuildSystemStatusCard()
    {
        var card = BorderedPanel();
        card.Margin = new Padding(5, 2, 4, 7);
        var layout = new TableLayoutPanel { Dock = DockStyle.Fill, ColumnCount = 1, RowCount = 9, Padding = new Padding(12, 9, 12, 9) };
        layout.RowStyles.Add(new RowStyle(SizeType.Absolute, 34));
    ...
    private Control StatusRow(string caption, Label value)
    {
        ...
        row.Controls.Add(new Label { Text = "●  " + caption, Dock = DockStyle.Fill, ForeColor = Color.FromArgb(190, 212, 234), Font = new Font("맑은 고딕", 8f), TextAlign = ContentAlignment.MiddleLeft }, 0, 0);
        value.Dock = DockStyle.Fill;
        value.ForeColor = Green;
        value.Font = new Font("맑은 고딕", 8.3f, FontStyle.Bold);
        value.TextAlign = ContentAlignment.MiddleRight;
        row.Controls.Add(value, 1, 0);
        return row;
    }
    ...
    private Control BuildAbyssDungeonGallery()
    {
        ...
        for (int i = 0; i < names.Length; i++)
        {
            ...
            tile.Paint += (_, e) =>
            {
                ...
                using var pen = new Pen(selected ? Accent : Line, selected ? 3f : 1f);
            ...
            inner.RowStyles.Add(new RowStyle(SizeType.Absolute, 29));
            inner.RowStyles.Add(new RowStyle(SizeType.Absolute, 24));
            inner.RowStyles.Add(new RowStyle(SizeType.Absolute, 32));
            var picture = new PictureBox { Dock = DockStyle.Fill, SizeMode = PictureBoxSizeMode.Zoom, BackColor = Color.FromArgb(5, 19, 34), Cursor = Cursors.Hand };
            try
            {
    ...
    {
        if (_bottomHost is null || _logView is null) return;
        _logView.Visible = _logExpanded;
        _bottomHost.Height = _logExpanded ? 225 : 58;
        if (_logToggle is not null) _logToggle.Text = _logExpanded ? "▤  로그 접기" : "▤  로그 보기 (최근 기록)";
        if (_logExpanded) _logView.BringToFront();
    }
    ...
    private void UpdateDashboard()
    {
        ...
        _currentStepValue.Text = string.IsNullOrWhiteSpace(_statusValue.Text) ? "준비" : _statusValue.Text;
...
'''
write_text(app / "MainForm.Dashboard.cs", dashboard)

# Windows assembly/file version.
p = app / "FishingAutomation.csproj"
s = read_text(p)
for key, value in {"Version":"48.0.0", "AssemblyVersion":"48.0.0.0", "FileVersion":"48.0.0.0"}.items():
    pattern = rf'<{key}>[^<]+</{key}>'
    replacement = f'<{key}>{value}</{key}>'
    if re.search(pattern, s): s = re.sub(pattern, replacement, s, count=1)
    else: s = s.replace("<PropertyGroup>", "<PropertyGroup>\n    " + replacement, 1)
write_text(p, s)

(root / "CHANGES_v48_UI.txt").write_text(
    "v48 UI-only rebuild on v47/v46 stable runtime.\n"
    "- Mabi_Auto dashboard rebuilt to approved reference layout\n"
    "- top four readiness chips\n"
    "- large status banner\n"
    "- four info cards: mode / dungeon / step / recent recognition\n"
    "- progress + compact logs + execution statistics\n"
    "- right system status including watchdog and update\n"
    "- Abyss image cards using existing v46 templates\n"
    "- collapsible log + large start/stop button\n"
    "- potion popup close remains active in background and is not displayed\n"
    "- no scenario, target, OCR, click, timing or recovery logic changed\n",
    encoding="utf-8-sig"
)
print("v48 UI-only dashboard rebuild complete; v47/v46 gameplay retained")
