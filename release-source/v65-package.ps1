...
$v65AuditPath = Join-Path $SourceRoot 'V65_ABYSS_POPUP_AUDIT.json'
if (-not (Test-Path -LiteralPath $v65AuditPath)) { throw 'V65 Abyss popup audit missing' }
$v65Audit = Get-Content -LiteralPath $v65AuditPath -Raw | ConvertFrom-Json
if ($v65Audit.version -ne 'v65') { throw 'Expected audited v65 source' }
foreach ($entry in $v65Audit.files.PSObject.Properties) {
    $path = Join-Path $SourceRoot $entry.Name
    if (-not (Test-Path -LiteralPath $path)) { throw "Audited v65 file missing: $($entry.Name)" }
    if ((Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash.ToLowerInvariant() -ne $entry.Value.ToLowerInvariant()) {
        throw "v65 source integrity mismatch: $($entry.Name)"
    }
}

$v58AuditPath = Join-Path $SourceRoot 'V58_PACKAGING_AUDIT.json'
if (-not (Test-Path -LiteralPath $v58AuditPath)) { throw 'V58 packaging audit missing from v65 source' }
...
$liteRoot = Join-Path $OutputRoot 'MabiAuto_v65'
...
@'
MABI AUTO v65 - Windows Lite

Abyss popup-monitor false-click guard:
- abyss_popup_close is now visual-template only
- OCR fallback for popup-close is disabled so unrelated text such as 나가기 cannot trigger the monitor
- popup monitor cooldown is increased to reduce repeat clicks
- existing Abyss scene-skip, treasure-chest, recovery and retry logic are otherwise unchanged
- v64 administrator elevation, exact 800x1000 sizing and dual-monitor handling remain intact
...
if ($version -ne '65.0.0.0') { throw "Unexpected executable version: $version" }
...
try {
    ...
    Write-Host 'STARTUP HEALTH OK: v65 WinForms app and Watchdog started.'
} finally {
...
$zip = Join-Path $OutputRoot 'MabiAuto_v65_Windows_Lite.zip'
Compress-Archive -LiteralPath $liteRoot -DestinationPath $zip -CompressionLevel Optimal
$hash = (Get-FileHash -LiteralPath $zip -Algorithm SHA256).Hash.ToLowerInvariant()
"$hash  MabiAuto_v65_Windows_Lite.zip" | Set-Content -LiteralPath "$zip.sha256" -Encoding ASCII
...
$inventory | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath (Join-Path $OutputRoot 'v65-package-inventory.json') -Encoding UTF8
