...
$v64AuditPath = Join-Path $SourceRoot 'V64_FINAL_CLEANUP_AUDIT.json'
if (-not (Test-Path -LiteralPath $v64AuditPath)) { throw 'V64 final cleanup audit missing' }
$v64Audit = Get-Content -LiteralPath $v64AuditPath -Raw | ConvertFrom-Json
if ($v64Audit.version -ne 'v64') { throw 'Expected audited v64 source' }
foreach ($entry in $v64Audit.files.PSObject.Properties) {
    $path = Join-Path $SourceRoot $entry.Name
    if (-not (Test-Path -LiteralPath $path)) { throw "Audited v64 file missing: $($entry.Name)" }
    if ((Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash.ToLowerInvariant() -ne $entry.Value.ToLowerInvariant()) {
        throw "v64 source integrity mismatch: $($entry.Name)"
    }
}

$v58AuditPath = Join-Path $SourceRoot 'V58_PACKAGING_AUDIT.json'
if (-not (Test-Path -LiteralPath $v58AuditPath)) { throw 'V58 packaging audit missing from v64 source' }
...
$liteRoot = Join-Path $OutputRoot 'MabiAuto_v64'
...
@'
MABI AUTO v64 - Windows Lite

Final cleanup release after confirming the real resize cause:
- MABI AUTO requests administrator privileges automatically at launch
- exact game client target remains 800x1000
- primary-monitor top-right placement and dual-monitor handling remain enabled
- visible game-window selection remains enabled
- redundant MoveWindow/WINDOWPLACEMENT retry behavior is no longer on the active resize path
- updater/watchdog, OCR/retry detection and safe packaging remain unchanged

Windows shows a UAC confirmation when MABI AUTO starts. This is required because Windows returned Win32 error 5 (Access Denied) when MABI AUTO was not elevated while the game was elevated.
...
if ($version -ne '64.0.0.0') { throw "Unexpected executable version: $version" }
...
try {
    ...
    Write-Host 'STARTUP HEALTH OK: v64 WinForms app and Watchdog started.'
} finally {
...
$zip = Join-Path $OutputRoot 'MabiAuto_v64_Windows_Lite.zip'
Compress-Archive -LiteralPath $liteRoot -DestinationPath $zip -CompressionLevel Optimal
$hash = (Get-FileHash -LiteralPath $zip -Algorithm SHA256).Hash.ToLowerInvariant()
"$hash  MabiAuto_v64_Windows_Lite.zip" | Set-Content -LiteralPath "$zip.sha256" -Encoding ASCII
...
$inventory | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath (Join-Path $OutputRoot 'v64-package-inventory.json') -Encoding UTF8
