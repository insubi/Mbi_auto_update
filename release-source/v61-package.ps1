...
$v61AuditPath = Join-Path $SourceRoot 'V61_WINDOW_STATE_AUDIT.json'
if (-not (Test-Path -LiteralPath $v61AuditPath)) { throw 'V61 window-state audit missing' }
$v61Audit = Get-Content -LiteralPath $v61AuditPath -Raw | ConvertFrom-Json
if ($v61Audit.version -ne 'v61') { throw 'Expected audited v61 source' }
foreach ($entry in $v61Audit.files.PSObject.Properties) {
    $path = Join-Path $SourceRoot $entry.Name
    if (-not (Test-Path -LiteralPath $path)) { throw "Audited v61 file missing: $($entry.Name)" }
    if ((Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash.ToLowerInvariant() -ne $entry.Value.ToLowerInvariant()) {
        throw "v61 source integrity mismatch: $($entry.Name)"
    }
}

$v58AuditPath = Join-Path $SourceRoot 'V58_PACKAGING_AUDIT.json'
if (-not (Test-Path -LiteralPath $v58AuditPath)) { throw 'V58 packaging audit missing from v61 source' }
...
$liteRoot = Join-Path $OutputRoot 'MabiAuto_v61'
...
@'
MABI AUTO v61 - Windows Lite

v61 fixes a window state case seen in real use:
- if the game is maximized, restore it before resizing
- then force actual client area to 800x1000
- place it at the PRIMARY monitor top-right
- re-measure repeatedly after state/DPI changes
Fishing, Dungeon and Abyss use the same required geometry.
v60 dual-monitor handling, v58 packaging and v57 updater/watchdog fixes remain intact.
...
if ($version -ne '61.0.0.0') { throw "Unexpected executable version: $version" }
...
try {
    ...
    Write-Host 'STARTUP HEALTH OK: v61 WinForms app and Watchdog started.'
} finally {
...
$zip = Join-Path $OutputRoot 'MabiAuto_v61_Windows_Lite.zip'
Compress-Archive -LiteralPath $liteRoot -DestinationPath $zip -CompressionLevel Optimal
$hash = (Get-FileHash -LiteralPath $zip -Algorithm SHA256).Hash.ToLowerInvariant()
"$hash  MabiAuto_v61_Windows_Lite.zip" | Set-Content -LiteralPath "$zip.sha256" -Encoding ASCII
...
$inventory | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath (Join-Path $OutputRoot 'v61-package-inventory.json') -Encoding UTF8
