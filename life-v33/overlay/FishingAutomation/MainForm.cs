...
public sealed partial class MainForm : Form
{
    ...
    private bool AnyRunning => _starting || _fishingBot.IsRunning || DungeonRunning || LifeRunning;
    ...
    {
        var button = new Button { Text = text, Dock = DockStyle.Fill, Margin = margin, FlatStyle = FlatStyle.Flat, BackColor = bg, ForeColor = fg, Font = new Font("Segoe UI", 11.5f, FontStyle.Bold), Cursor = Cursors.Hand, UseCompatibleTextRendering = true };
    ...
    private void StartSelected()
    {
        ...
        else if (SelectedMode == "생활") _ = StartLifeAsync();
    ...
    private async Task StartScenarioAsync(string modeName, string folderName)
    {
        ...
        try
        {
            ...
            if (settings.ForceClientSizeAndTopRight)
            {
            ...
            _dungeonTask = Task.Run(async () =>
            {
                using (engine)
                {
                    ...
                    {
                        Ui(() => { _runError = ex.Message; _logExpanded = true; ApplyLogVisibility(); _log.Write($"[{modeName}] 오류: " + ex.Message); _ = _notifier.SendAlertAsync($"{modeName} 매크로 오류", ex.Message, true); });
                    }
                    finally
                    {
                        Ui(() =>
        ...
        catch (Exception ex)
        {
            ...
            _ = _notifier.SendAlertAsync($"{modeName} 시작 실패", ex.Message, true);
        }
    ...
    private void ProcessScenarioLog(string modeName, string text)
    {
        ...
        if (text.StartsWith("[자동복구] 1/", StringComparison.OrdinalIgnoreCase) && text.Contains("단계 시간초과"))
            _ = _notifier.SendAlertAsync($"{modeName} 단계 자동복구 시작", text, true);
        UpdateStats();
    }
    ...
    private void TestOrRefresh()
    {
        ...
        if (SelectedMode == "생활")
        {
            ...
            RefreshModeStatus();
            _log.Write("[생활] " + (_lifeProblems.Count == 0 ? "이미지 설정 확인 완료" : string.Join(" / ", _lifeProblems)));
            return;
        }
        if (SelectedMode == "낚시")
        {
    ...
    private void RefreshModeStatus()
    {
        ...
        if (SelectedMode == "생활")
        {
            _inputValue.Text = "기존 게임 창 · Interception · 한국어 OCR";
            SetStatus(_lifeProblems.Count == 0 ? "생활 준비 완료" : "생활 이미지 준비 필요", _lifeProblems.Count == 0 ? Green : Color.Orange);
            return;
        }
        if (SelectedMode == "낚시")
        {
    ...
    private void UpdateStats()
    {
        ...
        if (mode == "생활") { UpdateLifeStats(); return; }
        if (mode == "던전" || mode == "어비스")
        {
    ...
    private void TrackAlertHealthFromLog(string line)
    {
        ...
        if (text.Contains("네트워크 재시도", StringComparison.OrdinalIgnoreCase))
        {
            ...
            if (_networkRetryCount == limit)
                _ = _notifier.SendAlertAsync("네트워크 오류 반복", $"네트워크 재시도가 {_networkRetryCount}회 연속 발생했습니다. 현재 모드: {_activeMode ?? SelectedMode}", true);
        }

        if (text.Contains("퇴장 절차 시작"))
        {
            _exitProcedureDeadline = DateTime.Now.AddSeconds(Math.Max(30, _notifier.Settings.ExitProcedureTimeoutSeconds));
            _exitProcedureAlerted = false;
        }
        if (text.Contains("강제 퇴장 완료") || text.Contains("판 완료"))
        {
            _exitProcedureDeadline = null;
            _exitProcedureAlerted = false;
            _networkRetryCount = 0;
    ...
    private void CheckAlertHealth()
    {
        if (_fishingBot.IsRunning)
        {
            int seconds = Math.Max(30, _notifier.Settings.FishingStallSeconds);
            if (!_fishingStallAlerted && (DateTime.Now - _lastFishingProgressAt).TotalSeconds >= seconds)
            {
                _fishingStallAlerted = true;
                _ = _notifier.SendAlertAsync("낚시 매크로 진행 정지 감지", $"{seconds}초 이상 정상 진행 신호가 없습니다.", true);
            }
        }
        ...
        if (DungeonRunning && _exitProcedureDeadline.HasValue && !_exitProcedureAlerted && DateTime.Now >= _exitProcedureDeadline.Value)
        {
            ...
            _ = _notifier.SendAlertAsync("던전 강제 퇴장 지연", "10분 제한 후 시작된 퇴장 절차가 설정 시간 안에 완료되지 않았습니다.", true);
        }
    }
